    ...
    <tr className="w-full">
      <td>
        <Link href={`/product/${item.ProductId}`}>
          <Image src={item.imageLink} alt={item.title} width={80} height={80} />
        </Link>
        <div className="max-lg:hidden max-md:inline whitespace-nowrap text-lg pb-5 ">
          <Link href={`/product/${item.ProductId}`}>
            <p>{item.title}</p>
          </Link>
        </div>
        <div className=" mt-2 flex-col hidden items-center max-sm:flex">
          <div className="flex">
