...
export const removeItemFromLocalStorageCartList = (id) => {
  const localStorageCartList = getLocalStorageCartItems();

  const newItems = localStorageCartList.filter((item) => {
    if (item._id !== id) {
      return item;
    }
  });
...
export const changeLocalStorageCartItemAmount = (id, amount) => {
  const localStorageCartList = getLocalStorageCartItems();

  const changedArray = localStorageCartList.map((item) => {
    if (item._id === id) {
...
export const resetLocalStorageCartItems = () => {
